...
static void print_subtree(node * t,int indent)
  {
//  register int spaces;
static int spaces;

/*
    if (t->info=="+") cout << "plus" << t->info << endl;
    if (t->info=="-") cout << "minus" << t->info << endl;
    if (t->info=="*") cout << "times" << t->info << endl;
    if (t->info=="/") cout << "divide" << t->info << endl;
*/
...
//  while(TreeOper(t) != NumberToken)
  ...
  for(spaces = 0; spaces != indent; spaces++)
    putchar(' ');
    cout << " "  << " " << t->array[0] << " " << t->array[1] << " " << t->array[2] << " " <<endl;
...
*/
    ...
    print_tree(root);
    cout << endl <<  "Evaluation of parse tree: " << endl;
    cout << eval2(root);
